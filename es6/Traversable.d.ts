...
export interface Traverse<T> {
    <F extends URIS3>(F: Applicative3<F>): <A, FR, FE, B>(f: (a: A) => Kind3<F, FR, FE, B>) => (ta: HKT<T, A>) => Kind3<F, FR, FE, HKT<T, B>>;
    <F extends URIS3, FE>(F: Applicative3C<F, FE>): <A, FR, B>(f: (a: A) => Kind3<F, FR, FE, B>) => (ta: HKT<T, A>) => Kind3<F, FR, FE, HKT<T, B>>;
    <F extends URIS2>(F: Applicative2<F>): <A, FE, B>(f: (a: A) => Kind2<F, FE, B>) => (ta: HKT<T, A>) => Kind2<F, FE, HKT<T, B>>;
    <F extends URIS2, FE>(F: Applicative2C<F, FE>): <A, B>(f: (a: A) => Kind2<F, FE, B>) => (ta: HKT<T, A>) => Kind2<F, FE, HKT<T, B>>;
...
export interface Traverse2C<T extends URIS2, TE> {
    <F extends URIS3>(F: Applicative3<F>): <A, FR, FE, B>(f: (a: A) => Kind3<F, FR, FE, B>) => (ta: Kind2<T, TE, A>) => Kind3<F, FR, FE, Kind2<T, TE, B>>;
    <F extends URIS2>(F: Applicative2<F>): <A, FE, B>(f: (a: A) => Kind2<F, FE, B>) => (ta: Kind2<T, TE, A>) => Kind2<F, FE, Kind2<T, TE, B>>;
    <F extends URIS2, FE>(F: Applicative2C<F, FE>): <A, B>(f: (a: A) => Kind2<F, FE, B>) => (ta: Kind2<T, TE, A>) => Kind2<F, FE, Kind2<T, TE, B>>;
    <F extends URIS>(F: Applicative1<F>): <A, B>(f: (a: A) => Kind<F, B>) => (ta: Kind2<T, TE, A>) => Kind<F, Kind2<T, TE, B>>;
...
export interface Traverse3<T extends URIS3> {
    <F extends URIS3>(F: Applicative3<F>): <A, FR, FE, B>(f: (a: A) => Kind3<F, FR, FE, B>) => <TR, TE>(ta: Kind3<T, TR, TE, A>) => Kind3<F, FR, FE, Kind3<T, TR, TE, B>>;
    <F extends URIS2>(F: Applicative2<F>): <A, FE, B>(f: (a: A) => Kind2<F, FE, B>) => <TR, TE>(ta: Kind3<T, TR, TE, A>) => Kind2<F, FE, Kind3<T, TR, TE, B>>;
    <F extends URIS2, FE>(F: Applicative2C<F, FE>): <A, B>(f: (a: A) => Kind2<F, FE, B>) => <R, TE>(ta: Kind3<T, R, TE, A>) => Kind2<F, FE, Kind3<T, R, TE, B>>;
    <F extends URIS>(F: Applicative1<F>): <A, B>(f: (a: A) => Kind<F, B>) => <R, TE>(ta: Kind3<T, R, TE, A>) => Kind<F, Kind3<T, R, TE, B>>;
    <F>(F: Applicative<F>): <A, B>(f: (a: A) => HKT<F, B>) => <R, TE>(ta: Kind3<T, R, TE, A>) => HKT<F, Kind3<T, R, TE, B>>;
}
/**
 * @since 3.0.0
 */
...
export interface Sequence2<T extends URIS2> {
    <F extends URIS3>(F: Applicative3<F>): <TE, FR, FE, A>(ta: Kind2<T, TE, Kind3<F, FR, FE, A>>) => Kind3<F, FR, FE, Kind2<T, TE, A>>;
...
export interface Sequence2C<T extends URIS2, TE> {
    <F extends URIS3>(F: Applicative3<F>): <FR, FE, A>(ta: Kind2<T, TE, Kind3<F, FR, FE, A>>) => Kind3<F, FR, FE, Kind2<T, TE, A>>;
    <F extends URIS2>(F: Applicative2<F>): <FE, A>(ta: Kind2<T, TE, Kind2<F, FE, A>>) => Kind2<F, FE, Kind2<T, TE, A>>;
    <F extends URIS2, FE>(F: Applicative2C<F, FE>): <A>(ta: Kind2<T, TE, Kind2<F, FE, A>>) => Kind2<F, FE, Kind2<T, TE, A>>;
...
export interface Sequence3<T extends URIS3> {
    ...
    <F extends URIS2>(F: Applicative2<F>): <TR, TE, FE, A>(ta: Kind3<T, TR, TE, Kind2<F, FE, A>>) => Kind2<F, FE, Kind3<T, TR, TE, A>>;
    <F extends URIS2, FE>(F: Applicative2C<F, FE>): <TR, TE, A>(ta: Kind3<T, TR, TE, Kind2<F, FE, A>>) => Kind2<F, FE, Kind3<T, TR, TE, A>>;
